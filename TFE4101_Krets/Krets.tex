...
polynomet. Eksempel:
\begin{align*}
  1100101_2 &= 1\cdot2^6 + 1\cdot2^5 + 0\cdot2^4 + 0\cdot2^3 + 1\cdot2^2 + 0\cdot2^1 + 1\cdot2^0 \\
            &= 64 + 32 + 4 + 1 \\
            &= 101_{10}
\end{align*}

\subsection{Konvertere til binærtall}
For å konvertere fra 10-tallsystemet til binærtall kan man enten begynne
fra MSB eller LSB. Vi ser på sistnevnte. LSB er 1 hvis of bare hvis tallet er et
oddetall. Etter å ha funnet LSB heltallsdividerer vi på 2 og fortsetter på samme
måte for å finne neste bit. Eksempel med $73_{10}$:
\begin{align*}
  & 73 \text{\ er et oddetall} & \rightarrow 1 & \textit{(LSB)}\\
  \floor*{\frac{73}{2}} =\ & 36 \text{\ er et partall} & \rightarrow 0 & \\
  \floor*{\frac{36}{2}} =\ & 18 \text{\ er et partall} & \rightarrow 0 & \\
  \floor*{\frac{18}{2}} =\ & 9 \text{\ er et oddetall} & \rightarrow 1 & \\
  \floor*{\frac{9}{2}} =\ & 4 \text{\ er et partall} & \rightarrow 0 & \\
  \floor*{\frac{4}{2}} =\ & 2 \text{\ er et partall} & \rightarrow 0 & \\
  \floor*{\frac{2}{2}} =\ & 1 \text{\ er et oddetall} & \rightarrow 1 & \textit{(MSB)}
\end{align*}
...
sannhetstabellen, og timingdiagrammet (TODO). Se Figur~\ref{fig:SR-flip-flop-block} og Tabell~\ref{tab:SR-flip-flop-truth}.
