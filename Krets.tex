...
\begin{center}
  \fbox{\begin{minipage}{.8\textwidth}
    ...
    nivåstyrt, og \textit{vippe} om det som er flankestyrt. Dette er basert på
    diverse engelske definisjoner av henholdsvis \textit{latch} og
    \textit{flip-flop}. 
  \end{minipage}}
\end{center}
...
Disse blir SR-låsens outputs, $Q$ og $Q'$. Se Figur~\ref{fig:SR-latch}.
...
ha lav output. $Q$ og $Q'$ er altså motsatte under vanlig drift. Med $S$ og $R$
kan man styre hvilke NOR-porter som gjør hva. Når $S$ blir satt høy vil
NOR-porten til $Q'$ bli lav, som setter NOR-porten til $Q$ høy, som igjen går
inn i $Q'$ sin NOR-port og holder den lav. Motsatte skjer for $R$. Resultatet er
...
Dersom både $S$ og $R$ er høye samtidig blir både $Q$ og $Q'$ $0$, og hvis begge
inputs går lave samtidig blir tilstanden ubestemt.
...
\subsection{D-lås}
En D-lås er en SR-lås men med en enkel data-inngang $D$ og en enable-inngang
$EN$. Kretsen virker som en styrt SR-lås, men $S$ kommer fra $D$, og $R$ kommer
fra $\bar{D}$. Se Figur~\ref{fig:D-latch}.

\begin{figure}[hbt!]
  \centering
  \begin{minipage}{.50\textwidth}
    \begin{circuitikz}[scale=.7,transform shape] \draw
      (3,3) node[american nor port](R_NOR){}
      (3,0) node[american nor port](S_NOR){}
      ...
      (R_AND.in 1) ++(-1,0) node[american not port](NOT){}
      (R_AND.in 1) ++(-2,0) coordinate(R_NOT_IN)
      (R_AND.in 1) -- (NOT.out)

      (S_AND.in 2) -- ++(-2,0) node[circ](D){} -- (R_NOT_IN) -- (NOT.in)
      (D) -- ++(-1,0) node[left]{$D$}
      ;
    \end{circuitikz}
  ...
  \begin{minipage}{.3\textwidth}
    ...
    \begin{tabular}{ccc|c}
      ...
      \bottomrule
    \end{tabular}
    \captionof{table}{Sannhetstabell for D-lås\label{tab:D-latch}}
  \end{minipage}
\end{figure}
...
Vi instroduserer et klokkesignal

\subsection{Styrt SR-lås}
TODO\@: Er dette en greie? klokke

\subsection{Styrt D-lås}
En SR men S og R kommer begge fra samme input D, der S(?) = D'.
Begge inputene er ANDet med en input C. Den tar altså kun inn ny verdi når C er
høy. Viktig at C ikke går til lav mens D-invers enda ikke er propagert til S.
...
For flankestyrte kretser er $t_{hold}$ holdetiden etter trigger-flanken.
...
det stigende klokkeflanke, og motsatt høy til lav kalles synkende klokkeflanke.
...
I blokkdiagrammer har man et symbol for å vise at
triggring skjer på stigende klokkeflanke, se Figur~\ref{fig:trigger_shift}. Hvis
triggring skjer på synkende flanke vil klokkeinputen ha en sirkel foran seg,
likt en pMOS\@.

\subsection{Flankestyrt D-vippe (Master-slave-vippe)}
...
\begin{figure}[hbt!]
  ...
  \caption{To D-låser i master-slave-oppsett\label{fig:master_slave}}
\end{figure}
